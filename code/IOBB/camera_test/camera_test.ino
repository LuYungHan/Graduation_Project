...
//      Serial.println(typeof(dataBuffer[320]));
//        string s = System.Text.Encoding.UTF8.GetString(dataBuffer,0,dataBuffer.Length);
    while (x--){
...
//            Serial.println((PINC & 15));
//            Serial.println((PIND & 24));
          while (!(UCSR0A & (1 << UDRE0)));//wait for byte to transmit
...
//    char* s(reinterpret_cast<char*>(dataBuffer), sizeof(dataBuffer));
    //  while ((PIND & 256));//wait for low
    byte aaa;
//    for(int i=0;i<320;i++){
//      aBuffer1[i]dat = int(dataBuffer[i]);
//      
//      }
    Serial.print(",");
    for(int i=0;i<320;i++){
...
void setup(){
 ...
 writeReg(0x11, 10); //Earlier it had the value:writeReg(0x11, 12); New version works better for me :) !!!! 他也有微笑:)
